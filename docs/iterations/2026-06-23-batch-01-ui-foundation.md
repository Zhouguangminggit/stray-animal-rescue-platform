...
- **重构 `templates/components/site_header.html`**：移除硬编码的 "DjangoHarness" 品牌文案，保留完整的导航逻辑（登录/未登录状态、管理员入口、通知、我的救助/领养、退出）。
...
- **更新 `templates/accounts/auth_base.html`**：将硬编码的 "DjangoHarness" 品牌文案替换为 `{{ site_name }}` 变量，视觉栏文案改为平台使命描述。
- **更新 `templates/accounts/media.html`**：移除 alt 中的 "DjangoHarness"。
- **更新 `templates/accounts/register.html`**：将 "加入 DjangoHarness" 改为 "加入我们"。

### 验证结果
- 代码检查：无 CSS/HTML 文件使用已移除的旧变量（`--brand-brown`、`--brand-olive` 等）。
- 品牌文案：除 `README.md`、`README.en.md`、`skill/SKILL.md`（第十一批处理）和本项目文档外，模板中已无 "DjangoHarness" 硬编码文案。
